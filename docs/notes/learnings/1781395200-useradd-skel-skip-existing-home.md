---
title: useradd -m skips /etc/skel when home dir already exists (selkies: no .bashrc for desktop user)
date: 2026-06-13
tags: [devops, selkies, useradd, debian-image]
severity: low
---

## Problem
In the Selkies builder (`debian_qcow2-cloud-init-selkies.sh`) the desktop user
...
end of the `[USER]` block fixes ownership of the seeded files. Only the selkies builder
is affected — the cloud-init / no-cloud-init builders create no non-root user.

## References
- linux-debian/debian_qcow2-cloud-init-selkies.sh, `[USER]` block
